try {
    ...
    class SearchBar extends HTMLElement {
        constructor() {
            ...
            this.showOutOfStock = this.dataset.showOutOfStock === 'true';
            ...
            this.queries = [
                ...
                // Collections
                {
                    ...
                    params: {
                        ...
                        filters: 'NOT meta.algolia.hidden:true',
                        userToken: this.userToken,
                        clickAnalytics: true,
                    },
                },
        ...
        search() {
            ...
            `;
                        ...
                        result.hits.forEach((hit, index) => {
                            ...
                            const isHitInStock = hit.inventory_available !== false && hit.inventory_available !== 0 && hit.inventory_available !== 'false' && hit.inventory_available !== '0';
                            ...
                            const promo = this.getPromoTagFromHit(hit);

                            if (!isHitInStock) {
                                // Show SOLD OUT badge, hide promo
                                ...
                                product.appendChild(soldOutTag);
                            } else if (promo) {
                                // Only show promo when in stock
              ...
              `;
                            ...
                            if (hit.meta ? .algolia ? .all_variants ? .variants ? .length > 0) {
                                ...
                                cardSwatches.setLocation('search_bar');
                                ...
                                hit.option_names.forEach((option, optionIndex) => {
                                    ...
                                    if (
                                        ...
                                        if (swatchData.length > this.maxSwatches) {
                                            ...
                                            const remainingCount = swatchData.length - this.maxSwatches;
                                            const srText = `View ${remainingCount} more ${option} option${remainingCount === 1 ? '' : 's'}`;

                                            // Optional tooltip (mouse users). Not relied on for accessibility.
                                            moreSwatchesLink.title = srText;

                                            // Visible "+" plus accessible text for screen readers
              ...
              `;
                        ...
                        result.index == `${algoliaShopify.config.index_prefix}articles` &&
                        !result.params.includes('facetFilters=%5B%5B%22blog.title%3ASupport%20Articles%22%2C%22blog.title%3ACustomer%20Service%22%5D%5D')
                    ) {
        ...
        /**
         * Read the variant promo tag from the Algolia hit.
        ...
        getPromoTagFromHit(hit) {
            const promoTagHit = hit ? .meta ? .algolia ? .variant_promo_tag;
            if (!promoTagHit) return null;
            const promoTagContent = promoTagHit ? .value ? ? promoTagHit; // handle possible { value: {...} }
            if (!promoTagContent || typeof promoTagContent !== 'object') return null;

            const text = (promoTagContent.text || '').toString().trim();
            if (!text) return null;
            ...
            return {
                ...
                icon_url: promoTagContent.icon ? .url || '',
            };
        }
